...
import { ScreenHeader, StatusNote, TxLink, Copy, Refresh, Shield, Spinner, TokenIcon } from '../components/ui'
...
import { fetchStealthStatus, fetchAnnouncements, registerPending, fetchRelayInfo, type StealthStatus } from '@app/lib/stealth/api'
  ...
  return (
    <div className="space-y-3">
      <ScreenHeader
        ...
        right={
          <button
            className="press mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-edge bg-panel2 text-muted shadow-soft transition hover:border-gold/45 hover:text-gold disabled:opacity-40"
            onClick={() => void scan()}
            disabled={scanning || !keys}
            title="Rescan"
            aria-label="Rescan"
          >
            {scanning ? <Spinner size={12} /> : <Refresh width={13} height={13} />}
          </button>
        }
      />
      ...
      <section className="card">
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-muted">
          ...
          the pool — no consolidation, no link back to these addresses.
      ...
      <StatusNote status={busy ? (note ?? 'Working…') : null} error={error} />
      {txHash && <TxLink hash={txHash} label="Shielded to vault" />}
    </div>
  )
}
